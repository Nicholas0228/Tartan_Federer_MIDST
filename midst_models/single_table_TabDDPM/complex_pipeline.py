...
def clava_load_pretrained(relation_order, save_dir):
    models = {}
    for parent, child in relation_order:
        print(os.path.join(save_dir, f"models/{parent}_{child}_ckpt.pkl"))
...
def clava_load_pretrained_customized(relation_order, prefix = "/models", save_dir=None):
    models = {}
    for parent, child in relation_order:
        print(os.path.join(save_dir, f"{prefix}{parent}_{child}_ckpt.pkl"))
        assert os.path.exists(
            os.path.join(save_dir, f"{prefix}{parent}_{child}_ckpt.pkl")
        )
        print(f"{parent} -> {child} checkpoint found, loading...")
        with open(os.path.join(save_dir, f"{prefix}{parent}_{child}_ckpt.pkl"), "rb") as f:
            models[(parent, child)] = CustomUnpickler(f).load()
